...
resource "azurerm_resource_group" "this" {
  name     = "rg-network-${var.solution_details.name}-${var.solution_details.environment}"
  location = var.solution_details.location
...
module "network" {
  ...
  network = {
    # vnet configuration
    name                    = "vnet-${each.key}-${var.solution_details.name}-${var.solution_details.environment}"
...
resource "azurerm_route_table" "this" {
  ...
  name                          = "rt-${each.key}-${var.solution_details.name}-${var.solution_details.environment}"
  resource_group_name           = azurerm_resource_group.this.name
      ...
      for route_table, route_table_config in local.route_tables : [
        route_table_config.routes == null ? [] : [
          for route, route_config in route_table_config.routes : {
            ...
            name                   = "route-${route}-${var.solution_details.name}-${var.solution_details.environment}"
            route_table_name       = "rt-${route_table}-${var.solution_details.name}-${var.solution_details.environment}"
...
resource "azurerm_network_security_group" "this" {
  ...
  name                = "nsg-${each.key}-${var.solution_details.name}-${var.solution_details.environment}"
  location            = azurerm_resource_group.this.location
  resource_group_name = azurerm_resource_group.this.name
      ...
      for nsg, nsg_config in local.nsg : [
        nsg_config.security_rules == null ? [] : [
          for rule, rule_config in nsg_config.security_rules : {
            key                                        = "${rule}.${nsg}"
            name                                       = "nsgr-${rule}-${var.solution_details.name}-${var.solution_details.environment}"
            ...
            network_security_group_name                = "nsg-${nsg}-${var.solution_details.name}-${var.solution_details.environment}"
          }
        ]
      ]
    ]
